namespace Sitecore.Feature.Accounts.Tests
{
  ...
  public class AccountsControllerTests
  {
    ...
    [Theory]
    [AutoDbData]
    public void _Login_LoggedIn_ShouldRedirect(Database db, [Content] DbItem item, [Frozen] IAccountRepository repo, LoginInfo info, INotificationService service, IAccountsSettingsService accountSetting)
    {
    ...
    [Theory]
    [AutoDbData]
    public void Login_ValidaData_ShouldRedirectToReturnUrl([Frozen] IAccountRepository repo, LoginInfo info, INotificationService service, IAccountsSettingsService accountSetting)
    {
      var controller = new AccountsController(repo, service, accountSetting, null, null);
      repo.Login(string.Empty, string.Empty).ReturnsForAnyArgs(x => true);
    ...
    [Theory]
    [AutoDbData]
    public void Login_ReturnUrlNotSet_ShouldRedirectToRoot(Database db, [Content] DbItem item, [Frozen] IAccountRepository repo, LoginInfo info, INotificationService service, IAccountsSettingsService accountSetting)
    {
    ...
    [Theory]
    [AutoDbData]
    public void Login_ValidData_ShouldRedirect(Database db, [Content] DbItem item, [Frozen] IAccountRepository repo, LoginInfo info, INotificationService service, [Frozen] IAccountsSettingsService accountSetting)
    {
      accountSetting.GetPageLinkOrDefault(Arg.Any<Item>(), Arg.Any<ID>(), Arg.Any<Item>()).Returns("/");
      var fakeSite = new FakeSiteContext(new StringDictionary
                                         {
