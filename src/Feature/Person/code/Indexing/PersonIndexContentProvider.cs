namespace Sitecore.Feature.Person.Indexing
{
  ...
  public class PersonIndexContentProvider : IndexContentProviderBase
  {
    ...
    public override IEnumerable<ID> SupportedTemplates => new[] {Templates.Employee.ID};

    public override Expression<Func<SearchResultItem, bool>> GetQueryPredicate(IQuery query)
    {
      var fieldNames = new[] {Templates.Person.Fields.Title_FieldName, Templates.Person.Fields.Summary_FieldName, Templates.Person.Fields.Name_FieldName, Templates.Employee.Fields.Biography_FieldName};
