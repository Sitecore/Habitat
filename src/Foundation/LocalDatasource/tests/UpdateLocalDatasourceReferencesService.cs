namespace Sitecore.Foundation.Datasource.Tests
{
  ...
  public class UpdateLocalDatasourceReferencesServiceTests
  {
    ...
    [Theory]
    [AutoDbData]
    public void UpdateAsync_ItemPassed_ShouldReplaceLinks([Substitute] LinkDatabase linkDb, Db db)
    {
      var datasourceItemId = ID.NewID;
      ...
      using (new Sitecore.FakeDb.Links.LinkDatabaseSwitcher(linkDb))
      {
        ...
        referenceReplacer.UpdateAsync();
        Jobs.JobManager.GetJob($"Resolving item references between source {AuditFormatter.FormatItem(sourceItem)} and target {AuditFormatter.FormatItem(targetItem)}.").Wait(int.MaxValue);
